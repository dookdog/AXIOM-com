/**
 * axiom-suggest — v4 (2026-08-23) "completion-first + qlog + 90-day prune"
 * v3 added the identifier-free query log (qlog in D1) and /suggest/select.
 * v4 adds the nightly scheduled prune per Lionel's retention ruling (90 days).
...
export default {
  ...
  async scheduled(event, env, ctx){
    // Retention ruling (Lionel, 2026-08-23): qlog rows older than 90 days are pruned nightly.
    try{
      await env.QLOG_DB.prepare("DELETE FROM qlog WHERE ts < datetime('now','-90 days')").run();
    }catch(_){ /* prune must never throw into the platform */ }
  }
};
